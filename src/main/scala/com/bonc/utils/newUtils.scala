...
import com.bonc.JavaUtils.JavaUtils
import com.bonc.serviceImpl.{GaussianTE, KrasKovTE}
...
/**
  ...
  */
object newUtils extends java.io.Serializable{
//object newUtils{
  ...
  //使用Gaussian计算,TransferEntropy.spark.TransferEntropy使用
...
//            array3d.append(k + "," + l + "," + delay + "," + calc11.computeContinuous(src, dest, 100))
  ...
  //使用Gaussian计算,ByRDD4
  def computeTwoSeriesTEsStringResultGaussianSrcName(srcName:String, hashMap: Map[String,Int], src: Array[scala.Double], dest: Array[scala.Double], base: Int, k_max: Int, k_tau: Int, l_max: Int, l_tau: Int, delay_max: Int): Array[String] = {
    var array3d = new ArrayBuffer[String]()
    var calc11 = new GaussianTE()
...
//    startWithIntercept: Boolean
  ...
  //根据用户提供的“传递熵因果关系”，确定阀值。
  //问题在于，如果rules的minTE都特别大，比如远大于默认值（0.1）,则网络会特别小，会错失关系
  //所以可以设置一个默认值defaultThreshold
  //如果minTe>defaultThreshold ,则threshold=minTe, 否则threshold=defaultThreshold
  def computeThreshold(rules:String):Double={
    0.0
  }
}
